 ...
 * {@link RULE_NAME_MAX}。原样保留一支此前放到 32,三段拼起来最坏 117。
 *
 * 派生名**因本次改动而变化**的 id 有两段,不是一段:
 *   · **27–32 位的合规 id** —— 越界的只有这一段,旧代码原样保留、拼得出 100 以上;
 *   · **33 位以上的合规 id** —— 旧代码走「头部 12 位 + 哈希」,现在改走「截断 + 8 位哈希」,
 *     派生名同样变了,只是它本来就没越界。
 * 两段在本应用里都不产生:平台记录 id 是 UUID(走第三支),应用自定义 id 是 `bu_market` 这类短名。
 *
