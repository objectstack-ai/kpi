 ...
 * - 到人:个人得分 = Σ(所属板块填报单得分 × 分工权重%) × 个人系数 + Σ(承接项得分率% × 承接权重%)
 *   ——承接项得分率取该下达指标在主体填报单里的得分率(疑点 Q-03 未拍板,按 kb 推断口径实现,
 *   口径改动只改本文件);
 * - 分管领导:分管主体的最终得分按主体权重加权平均(权重全 0 时算术平均)(疑点 Q-05 推断口径)。
 */
...
export function aggregateResults(
  ...
  const unitRows: ResultRow[] = sheets.map((s) => ({
    ...
    rank: 0,
    breakdown: { total_score: s.totalScore, subject_weight: subjectWeight.get(s.subjectId) ?? 0 },
  }));
  ...
  for (const a of assignments) {
    ...
    for (const it of a.personalItems) {
      ...
      fromItems += s;
      items.push({ plan_indicator: it.planIndicatorId, score_rate: rate, weight: it.weight, score: s });
    }
