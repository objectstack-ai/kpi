 ...
 *   - 设计:**能看到哪些行由数据层决定** —— 对象 OWD `private` / `controlled_by_parent` +
 *     方案发布时写入的动态共享规则 + 权限集 `readScope`。部门填报人员在「待人力审核」里
 *     本来就一行都读不到,分公司核对人员在「待我核对」里只读得到本分公司的任务;按岗位再
 *     显隐一次是把同一条边界画两遍,而画在前端的那一遍不是边界。视图筛选是展示范围,不是
 *     安全边界,这里不拿 `filter` 当权限用。
 *
